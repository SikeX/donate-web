  ...
  return (
    <div className="flex flex-col space-y-3 text-sm">
      <div className="flex space-x-2">
        ...
        {option.map(((item) => <Option id={item.id} name={item.name} money={parseFloat(item.money / 100)} getOptionMoney={getOptionMoney} />))}
      </div>
    ...
    <div className="w-full flex-grow flex flex-col">
      <div className="w-full py-8 md:px-24 flex flex-col md:flex-row">
        ...
        <div className="flex flex-col flex-grow px-3 justify-start space-y-3">
          <span className="text-gray-500 text-sm">{Date.now() > Date.parse(itemDetail.endTime) ? '已结束' : '筹集中'}</span>
          <div className="text-2xl font-bold">{itemDetail.name}</div>
          <div className="text-sm text-gray-500">{itemDetail.itemDesc}</div>
          <StatusBar target={parseFloat(itemDetail.targetMoney / 100)} raised={parseFloat(itemDetail.raisedMoney / 100)} support={supportNum} />
          {(itemDetail.endTime === null || Date.now() < Date.parse(itemDetail.endTime)) && <Donate id={id} option={option} leastMoney={itemDetail.leastMoney} />}
